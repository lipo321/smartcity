#include "OSGEventAdapter.h"


OSGEventAdapter::OSGEventAdapter(void)
{
}
...
void OSGEventAdapter::keyPressEvent(QKeyEvent *event)
{
	QPoint pos = QCursor::pos();

	QList<QGraphicsItem*>  listItems = items(mapToScene(pos.x(),
											pos.y()).toPoint());
...
void OSGEventAdapter::keyReleaseEvent(QKeyEvent* event)
{
	setKeyboardModifiers(event);
...
void OSGEventAdapter::mousePressEvent(QMouseEvent *event)
{
...
void OSGEventAdapter::mouseReleaseEvent(QMouseEvent *event)
{
	int button = 0;
	switch (event->button())
	{
...
void OSGEventAdapter::mouseDoubleClickEvent(QMouseEvent *event)
{
	QPoint pos = QCursor::pos();
	QList<QGraphicsItem*> listItems = items(mapToScene(pos.x(), pos.y()).toPoint());
	if (listItems.size() > 0)
...
void OSGEventAdapter::wheelEvent(QWheelEvent *event)
{
	setKeyboardModifiers(event);
...
void OSGEventAdapter::resizeEvent(QResizeEvent *event)
{
	if (scene())
	{
...
void OSGEventAdapter::moveEvent(QMoveEvent *event)
{
...
void OSGEventAdapter::timerEvent(QTimerEvent *event)
{
	scene()->update();
}

void OSGEventAdapter::setKeyboardModifiers(QInputEvent* event)
{
