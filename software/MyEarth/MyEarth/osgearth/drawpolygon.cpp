...
{
	if (nullptr != m_pFeatureNode)
	{
		delete m_pFeatureNode;
		m_pFeatureNode = nullptr;
	}
	
	if (nullptr != m_pStippleFeatureNode)
	{
		delete m_pStippleFeatureNode;
		m_pStippleFeatureNode = nullptr;
	}
	
	if (nullptr != m_pPolygonEdit )
	{
		delete m_pPolygonEdit;
		m_pPolygonEdit = nullptr;
	}
}
...
void DrawPolygon::slotPicked(osg::Vec3d pos)
{
	...
	std::vector<osg::Vec3d>::iterator itn = m_vecPoints.begin();
	for ( ; itn != m_vecPoints.end(); itn++)
	{
		pGeometry->push_back(*itn);
	}
