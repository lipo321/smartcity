...
void Avatar::addAvatar()
{
	osg::Node* pNode = osgDB::readNodeFile(
		QString::fromUtf8("E:/OSGCore/Path/data/tree.ive").toLocal8Bit().data());
	if (!pNode)
	{
		return;
	}
...
void Avatar::setPostion(double lon, double lat, double alt)
{
	if (m_pAvatarNode)
	{
		...
		// by lipo
		osgEarth::Util::Viewpoint pv( "",lon, lat, alt, 0.0, -90.0, 1000/*, m_pMap3D->getSRS()*/ );
		m_pMap3D->getManipulator()->setViewpoint(osgEarth::Util::Viewpoint( pv ));
	}
}
